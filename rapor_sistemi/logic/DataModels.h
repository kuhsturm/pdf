...
struct FonksiyonTesti {
    ...
    bool isValid() const {
        return !linye.isEmpty() && nominalAkim > 0;
    }
    ...
    bool isKakrGroup() const {
        return linye.toUpper().contains("KAKR");
    }

    // 32A altı KAKR kontrolü (ANA SİGORTA muaf - Python'daki gibi)
    bool needs30maKakr() const {
        // ANA SİGORTA için 32A kuralı geçerli değil (Python: multi_pano_gui.py:498-499)
        if (isAnaSigorta) return false;
...
struct PanoData {
    ...
    int fonksiyonTestiSayisi() const {
        return fonksiyonTestleri.size();
    }
};
...
inline int kesitToIz(double kesit, int carpan = 1) {
    ...
    int baseIz = tablo.value(kesit, 0);
...
inline QPair<double, int> parseKesit(const QString& kesitStr) {
    if (kesitStr.contains('x') || kesitStr.contains('X')) {
        QStringList parts = kesitStr.toLower().split('x');
        if (parts.size() == 2) {
