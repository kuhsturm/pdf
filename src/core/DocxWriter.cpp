...
// Word namespace'leri
static const QString NS_W =
    "http://schemas.openxmlformats.org/wordprocessingml/2006/main";
static const QString NS_R =
    "http://schemas.openxmlformats.org/officeDocument/2006/relationships";
static const QString NS_WP =
    "http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing";
static const QString NS_A =
    "http://schemas.openxmlformats.org/drawingml/2006/main";
static const QString NS_PIC =
...
bool DocxWriter::extractTemplate(const QString &templatePath) {
  ...
  if (!zip.isReadable()) {
    m_errorString =
        QString("DOCX dosyası okunamadı (Arşiv bozuk veya erişim yok): %1")
            .arg(templatePath);
    return false;
  }
  ...
  for (const QZipReader::FileInfo &info : allFiles) {
    ...
    if (info.isDir) {
      ...
      if (!dir.mkpath(destInfo.path())) {
        m_errorString =
            QString("Dosya yolu oluşturulamadı: %1").arg(destInfo.path());
        return false;
      }
      ...
      if (file.error() != QFile::NoError) {
        m_errorString =
            QString("Dosya yazma hatası: %1").arg(file.errorString());
        return false;
      }
    }
  }
...
bool DocxWriter::loadDocumentXml() {
  ...
  // Binary olarak oku - encoding dönüşümü YAPMA!
  // QString dönüşümü UTF-8'i bozuyor, bu yüzden sadece QByteArray kullan
  m_xmlRawData = file.readAll();
  file.close();

  // DOM yükle (tablo satır sayma gibi read-only işlemler için)
  QString errorMsg;
  int errorLine, errorCol;
  if (!m_document.setContent(m_xmlRawData, true, &errorMsg, &errorLine,
                             &errorCol)) {
  ...
  qDebug() << "[loadDocumentXml] XML loaded, size:" << m_xmlRawData.size();
  return true;
}
...
bool DocxWriter::saveDocumentXml() {
  ...
  // Binary olarak yaz - orijinal encoding korunuyor
  // m_xmlRawData üzerinde replacePlaceholder değişiklikleri uygulandı
  file.write(m_xmlRawData);
  file.close();

  qDebug() << "[saveDocumentXml] XML saved, size:" << m_xmlRawData.size();
  return true;
}

void DocxWriter::replacePlaceholder(const QString &placeholder,
                                    const QString &value) {
  // QByteArray üzerinde doğrudan binary replacement
  // Bu yöntem UTF-8 encoding'i korur

  if (placeholder.isEmpty())
    return;

  // GÜVENLİK KORUMASI:
  // "en" ve "boy" gibi kısa kelimeler XML tag'lerinin içinde geçebilir (örn:
  // <w:document> -> <w:docum1t>) Bu nedenle bunları engelle.
  if (placeholder == "en" || placeholder == "boy") {
    qWarning() << "[DocxWriter] Dangerous placeholder ignored:" << placeholder;
    return;
  }

  // Placeholder ve değeri UTF-8 byte dizisine çevir
  QByteArray placeholderBytes = placeholder.toUtf8();

  // Değeri XML escape et (örn & -> &amp;)
  QString escapedValue = escapeXml(value);
  QByteArray valueBytes = escapedValue.toUtf8();

  // Binary replacement
  if (m_xmlRawData.contains(placeholderBytes)) {
    m_xmlRawData.replace(placeholderBytes, valueBytes);
    qDebug() << "[replacePlaceholder]" << placeholder << "->" << escapedValue;
  }
}

QString DocxWriter::escapeXml(const QString &text) {
  QString result = text;
  result.replace("&", "&amp;")
      .replace("<", "&lt;")
      .replace(">", "&gt;")
...
void DocxWriter::setTableCell(int tableIndex, int rowIndex, int colIndex,
  ...
  // Python: para.paragraph_format.space_after = Pt(0), space_before = Pt(0),
  // line_spacing = 1.0 Mevcut pPr varsa kaldır, yenisini ekle
...
int DocxWriter::copyTableRow(int tableIndex, int sourceRowIndex) {
  ...
  // Python'daki _copy_row fonksiyonu: "Tüm hücrelerin içeriğini temizle ama
  // formatı koru"
    ...
    if (cellChild.isElement() && cellChild.toElement().localName() == "tc") {
      ...
      while (!paraChild.isNull()) {
        if (paraChild.isElement() && paraChild.toElement().localName() == "p") {
          ...
          QDomNode runChild = para.firstChild();
          while (!runChild.isNull()) {
            if (runChild.isElement() &&
                runChild.toElement().localName() == "r") {
              ...
              while (!textChild.isNull()) {
                if (textChild.isElement() &&
                    textChild.toElement().localName() == "t") {
...
void DocxWriter::fillFirmaBilgileri(const FirmaBilgileri &firma) {
  ...
  replacePlaceholder("baslangic_tarih_saat",
                     firma.baslangicTarihSaat.toString("dd.MM.yyyy HH:mm"));
  replacePlaceholder("bitis_tarih_saat",
                     firma.bitisTarihSaat.toString("dd.MM.yyyy HH:mm"));
  replacePlaceholder("bir_sonraki_kontrol",
                     firma.birSonrakiKontrol.toString("dd.MM.yyyy"));
...
void DocxWriter::fillAnaDagitimPano(const AnaDagitimPano &pano,
  ...
  // ÖNEMLİ: En uzun placeholder'dan başlayarak değiştir!
  // Aksi takdirde "t_n" değişince "t_n_c-s" bozulur!
  ...
  // 1. En uzunlar (Specific)
  replacePlaceholder("t_n_c-s", isTNCS ? "[X]" : "[ ]");

  replacePlaceholder("t_n-c", isTNC ? "[X]" : "[ ]");
  replacePlaceholder("t_n_c", isTNC ? "[X]" : "[ ]"); // Legacy support

  replacePlaceholder("t_n-s", isTNS ? "[X]" : "[ ]");

  // 2. Kısalar (Generic)
  replacePlaceholder("t_n", isTN ? "[X]" : "[ ]"); // Bu en son yapılmalı!
  ...
  // --- EKSİK OLAN PROJE VE TEK HAT ŞEMASI BİLGİLERİ ---
  // Placeholder isimleri tahmin edilerek çoklu deneme yapılıyor
  bool projeVar = pano.projeVarMi;
  QString pVarStr = projeVar ? "[X]" : "[ ]";
  QString pYokStr = projeVar ? "[ ]" : "[X]";

  replacePlaceholder("proje_var", pVarStr);
  replacePlaceholder("proje_yok", pYokStr);
  replacePlaceholder("prj_var", pVarStr);
  replacePlaceholder("prj_yok", pYokStr);
  replacePlaceholder("p_var", pVarStr); // Olası kısa ad
  replacePlaceholder("p_yok", pYokStr); // Olası kısa ad

  bool tekHatVar = pano.tekHatSemasiVarMi;
  QString thVarStr = tekHatVar ? "[X]" : "[ ]";
  QString thYokStr = tekHatVar ? "[ ]" : "[X]";

  replacePlaceholder("tekhat_var", thVarStr);
  replacePlaceholder("tekhat_yok", thYokStr);
  replacePlaceholder("tek_hat_var", thVarStr);
  replacePlaceholder("tek_hat_yok", thYokStr);
  replacePlaceholder("th_var", thVarStr); // Olası kısa ad
  replacePlaceholder("th_yok", thYokStr); // Olası kısa ad
  // ----------------------------------------------------
...
void DocxWriter::fillCihazBilgileri(const FirmaBilgileri &firma) {
  ...
  QString termalAdi =
      !firma.termalCihazAdi.isEmpty() ? firma.termalCihazAdi : firma.cihaz1Adi;
  QString termalSeriNo =
      !firma.termalSeriNo.isEmpty() ? firma.termalSeriNo : firma.cihaz1SeriNo;
  QString termalKalTarihi = !firma.termalKalibrasyonTarihi.isEmpty()
                                ? firma.termalKalibrasyonTarihi
                                : firma.cihaz1KalibrasyonTarihi;
  QString termalKalGecerlilik = !firma.termalKalibrasyonGecerlilik.isEmpty()
                                    ? firma.termalKalibrasyonGecerlilik
                                    : firma.cihaz1KalibrasyonGecerlilik;
  QString termalKalNo = !firma.termalKalibrasyonNo.isEmpty()
                            ? firma.termalKalibrasyonNo
                            : firma.cihaz1KalibrasyonNo;
  ...
  QString olcumAdi =
      !firma.olcumCihazAdi.isEmpty() ? firma.olcumCihazAdi : firma.cihaz2Adi;
  QString olcumSeriNo =
      !firma.olcumSeriNo.isEmpty() ? firma.olcumSeriNo : firma.cihaz2SeriNo;
  QString olcumKalTarihi = !firma.olcumKalibrasyonTarihi.isEmpty()
                               ? firma.olcumKalibrasyonTarihi
                               : firma.cihaz2KalibrasyonTarihi;
  QString olcumKalGecerlilik = !firma.olcumKalibrasyonGecerlilik.isEmpty()
                                   ? firma.olcumKalibrasyonGecerlilik
                                   : firma.cihaz2KalibrasyonGecerlilik;
  QString olcumKalNo = !firma.olcumKalibrasyonNo.isEmpty()
                           ? firma.olcumKalibrasyonNo
                           : firma.cihaz2KalibrasyonNo;
...
void DocxWriter::fillGozleKontrol(
    const QVector<GozleKontrolMaddesi> &maddeler) {
  ...
  for (const auto &madde : maddeler) {
    ...
    QString placeholder =
        QString("GK_%1").arg(madde.maddeNo, 2, 10, QChar('0'));
...
void DocxWriter::fillFlukeBilgileri(const QString &fotoTarihi,
                                    const QString &fotoNo) {
...
void DocxWriter::fillFonksiyonTestleri(const QVector<FonksiyonTesti> &testler,
  ...
  // Header placeholder'larını değiştir (Fonksiyon testleri bölümündeki ölçüm
  // değerleri) Z_ln'den 380/Z_ln otomatik hesapla
  QString kisaDevre;
  ...
  const int FIRST_DATA_ROW =
      10; // Python'daki start_row = 10 (header'dan sonra veri satırları)
  ...
  // 11-12: PE Kesiti (merged), 13-14: Ib (merged), 15: Iz, 16-17: IΔ mA
  // (merged) 18: TΔ ms, 19: Sonuç

  // Kablo Akım Taşıma Kapasiteleri (Iz hesabı için)
  // Python mantığı: parse_kesit -> base kesit bul, iz_table'dan değer al,
  // factor ile çarp
  // Kablo Akım Taşıma Kapasiteleri (Iz hesabı için)
  // DataModels.h içindeki ortak fonksiyonları kullan
  auto calcIz = [](const QString &fazKesiti) -> QString {
    auto [kesit, carpan] = parseKesit(fazKesiti);
    int iz = kesitToIz(kesit, carpan);
    return (iz > 0) ? QString::number(iz) : "";
  };

  // Şablon satırını sakla (kopyalama için)
  int templateRow = FIRST_DATA_ROW;

  // Python: multi_pano_gui.py:4051-4064 - Ana pano RCD değerleri (fallback
  // için)
  ...
  for (int i = 0; i < testler.size(); ++i) {
    ...
    QString iz = test.akimKapasitesi > 0 ? QString::number(test.akimKapasitesi)
                                         : calcIz(test.fazKesiti);
    ...
    // 4) RCD boş ve KAKR checkbox'ları işaretlenmemiş → Ana pano değerlerini
    // kullan Python: multi_pano_gui.py:4122-4125
    ...
    setTableCell(TABLE_INDEX, rowIdx, 3, QString::number(test.kutupSayisi),
                 6); // Kutup sayısı
    setTableCell(TABLE_INDEX, rowIdx, 4, QString::number(test.nominalAkim),
                 6); // In(A)
    setTableCell(TABLE_INDEX, rowIdx, 5, test.icu.isEmpty() ? "6" : test.icu,
                 6);                                            // Icu
    ...
    setTableCell(TABLE_INDEX, rowIdx, 11, test.rcd.isEmpty() ? rcdMa : test.rcd,
                 6); // IΔ mA (RCD değeri veya hesaplanan)
    setTableCell(TABLE_INDEX, rowIdx, 12, rcdMs.isEmpty() ? test.rcdMs : rcdMs,
                 6);                                 // TΔ ms
    setTableCell(TABLE_INDEX, rowIdx, 13, sonuc, 6); // Sonuç
  }
}

void DocxWriter::fillPotansiyelDengelemeVeZemin(
    const PanoData &pano, const QVector<FonksiyonTesti> &fonksiyonTestleri,
  ...
  // En büyük topraklama kesitini fonksiyon testlerinden hesapla (Python
  // mantığı)
  ...
  replacePlaceholder("en_buyuk_top_kesit",
                     enBuyukTopKesit.isEmpty() ? "6" : enBuyukTopKesit);
  ...
  QString durum =
      zeminIzolasyonDurum.isEmpty() ? pano.izoUygunluk : zeminIzolasyonDurum;
  ...
  qDebug() << "[DEBUG] 6.3 Zemin İzolasyonu:" << "en=" << enVal
           << "boy=" << boyVal << "izo_direnci=" << izoDirenci
           << "izo_uygunluk=" << izoUygunluk;

  // Placeholder değiştirme
  // DIKKAT: "en" ve "boy" placeholder'ları document.xml içindeki "document",
  // "sectPr" gibi tag'leri bozduğu için KALDIRILDI. (en -> 1 yapınca document
  // -> docum1t oluyor) Şablon güncellenmeli: {{zemin_en}}, {{zemin_boy}}
  // kullanılmalı. replacePlaceholder("en", enVal); replacePlaceholder("boy",
  // boyVal);
...
QString DocxWriter::addImage(const QString &imagePath) {
  ...
  ++m_imageCounter;
  QString rId =
      QString("rId%1").arg(100 + m_imageCounter); // Offset to avoid conflicts
  ...
  QString destName =
      QString("image%1.%2").arg(m_imageCounter).arg(info.suffix());
  ...
  if (relsFile.open(QIODevice::ReadWrite)) {
    ...
    rel.setAttribute("Type", "http://schemas.openxmlformats.org/officeDocument/"
...
void DocxWriter::addThermalImages(const QVector<TermalGoruntu> &images) {
  ...
  // Direct child row iteration - elementsByTagNameNS recursive olduğu için
  // kullanmıyoruz
  ...
  for (const auto &img : images) {
    ...
    docPr.setAttribute("name",
                       QString("Termal Görüntü %1").arg(addedCount + 1));
...
QString DocxWriter::generateKusurAciklamasi(const QVector<Kusur> &kusurlar) {
  ...
  for (const auto &k : kusurlar) {
    aciklamalar.append(QString("- %1: %2 (%3)")
                           .arg(k.linye, k.kusurAciklamasi, k.kusurDerecesi));
  }
...
bool DocxWriter::save(const QString &outputPath) {
  // DOĞRU HYBRID YAKLAŞIM:
  // 1. Placeholder değişiklikleri: m_xmlContent üzerinde yapıldı
  // (replacePlaceholder)
  // 2. Tablo değişiklikleri: m_document (DOM) üzerinde yapıldı (setTableCell)
  //
  // İki değişikliği birleştirmek için:
  // - DOM'u m_xmlContent'ten yeniden yükle (placeholder değişiklikleri dahil)
  // - Tablo değişiklikleri zaten DOM'da (setTableCell hala eski DOM'da)
  // - Bu yaklaşım SORUNLU çünkü DOM tekrar yüklenince tablo değişiklikleri
  // kayboluyor
  //
  // DOĞRU ÇÖZÜM: DOM değişikliklerini m_xmlContent'e yaz

  // DOM tabanlı değişiklikler yapıldıysa (setTableCell, copyTableRow, vb.)
  // bunları da m_xmlContent'e dahil etmeliyiz
  // Ancak toByteArray() namespace sorunlarına neden oluyor

  // GEÇICI ÇÖZÜM: Sadece m_xmlContent'i kaydet (placeholder değişiklikleri)
  // Tablo değişiklikleri için TODO: raw XML manipulation gerekiyor

  qDebug() << "[save] Saving DOCX to:" << outputPath;
  qDebug() << "[save] XML content size:" << m_xmlContent.size();

  // document.xml'i kaydet (sadece placeholder değişiklikleri)
  if (!saveDocumentXml()) {
    qDebug() << "[save] Failed to save document.xml:" << m_errorString;
    return false;
  }

  // ZIP olarak paketle
  if (!createZip(outputPath)) {
    qDebug() << "[save] Failed to create ZIP:" << m_errorString;
    return false;
  }

  qDebug() << "[save] DOCX saved successfully";
  return true;
}
