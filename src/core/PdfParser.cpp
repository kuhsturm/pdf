...
PdfParser::PdfParser() { initPatterns(); }
...
void PdfParser::initPatterns() {
  ...
  m_patternSozlesmeId =
      QRegularExpression(R"(S[öo]zle[şs]me\s*[Ii]d\s*[:=]\s*(\d+))",
  ...
  // Tesisat sayısı
  m_patternTesisatSayisi =
      QRegularExpression(R"(Tesisat\s*Say[ıi]s[ıi]\s*[:=]\s*(\d+))",
                         QRegularExpression::CaseInsensitiveOption);
}
...
QString PdfParser::findValue(const QString &text,
...
SozlesmeData PdfParser::parseSozlesme(const QString &pdfPath) {
  ...
  if (!tesisatStr.isEmpty()) {
    ...
    if (data.tesisatSayisi < 1)
      data.tesisatSayisi = 1;
  }
...
SozlesmeData PdfParser::parseSozlesmeViaPython(const QString &pdfPath) {
  ...
  // Python script yolunu bul - birden fazla olası konum
  QString exePath = QCoreApplication::applicationDirPath();
  QStringList possiblePaths = {
      exePath + "/sozlesme_parser.py",          // exe yanında (EN ÖNCELİKLİ)
      exePath + "/../sozlesme_parser.py",       // exe üst dizini
      exePath + "/../../sozlesme_parser.py",    // 2 üst dizin
      exePath + "/../../../sozlesme_parser.py", // 3 üst dizin
      "sozlesme_parser.py",                                  // çalışma dizini
  };
  ...
  for (const QString &path : possiblePaths) {
    if (QFile::exists(path)) {
      scriptPath = path;
      qDebug() << "[PdfParser] Python script found:" << path;
      break;
    }
  }

  if (scriptPath.isEmpty()) {
    qDebug() << "[PdfParser] Python script bulunamadı! Aranan yollar:";
    for (const QString &path : possiblePaths) {
      qDebug() << "  -" << path;
    }
    return data;
  ...
  // Encoding sorununu çözmek için UTF-8 zorla (Windows console encoding'den
  // bağımsız)
  ...
  for (const QString &line : lines) {
    if (line.contains(':')) {
      ...
      if (key == "sozlesme_id")
        data.sozlesmeId = value;
      else if (key == "sozlesme_baslangic")
        data.sozlesmeBaslangic = value;
      else if (key == "sozlesme_bitis")
        data.sozlesmeBitis = value;
      else if (key == "firma_unvan")
        data.firmaAdi = value;
      else if (key == "firma_adres")
        data.firmaAdres = value;
      else if (key == "firma_il")
        data.firmaIl = value;
      else if (key == "firma_sgk_no")
        data.sgkSicil = formatSgkNo(value);
      else if (key == "kontrol_eden_adsoyad")
        data.kontrolEdenAdSoyad = value;
      else if (key == "kontrol_eden_tc")
        data.kontrolEdenTc = value;
      else if (key == "pk_no")
        data.pkNo = value;
