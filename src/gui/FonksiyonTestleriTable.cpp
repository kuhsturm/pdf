...
FonksiyonTestleriModel::FonksiyonTestleriModel(QObject *parent)
  ...
  m_kesitValues = {"1.5",  "2.5",  "4",    "6",    "10",   "16",   "25",
                   "35",   "50",   "70",   "95",   "120",  "2x16", "2x25",
                   "2x35", "3x16", "3x25", "3x35", "4x16", "4x25"};
}
...
QVariant FonksiyonTestleriModel::data(const QModelIndex &index,
                                      int role) const {
  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    ...
    case ColLinye:
      return test.linye;
    case ColAcmaEgrisi:
      return test.sigortaTipi; // "Eğri" alanı
    case ColKutupSayisi:
      return test.kutupSayisi;
    case ColIn:
      return test.nominalAkim;
    case ColIcu:
      return test.icu;
    case ColIb:
      return test.ib;
    case ColFazKesiti:
      return test.fazKesiti;
    case ColNotrKesiti:
      return test.notrKesiti;
    case ColToprakKesiti:
      return test.toprakKesiti;
    case ColIz:
      return test.akimKapasitesi;
    case ColSonuc:
      return test.sonuc;
    case ColKakrVar:
      return test.kakrVar;
    ...
    if (index.column() == ColIn || index.column() == ColIz ||
        ...
        index.column() == ColIb || index.column() == ColRcdMa ||
        index.column() == ColRcdMs) {
...
bool FonksiyonTestleriModel::setData(const QModelIndex &index,
  ...
  switch (index.column()) {
  case ColLinye:
    test.linye = value.toString();
    break;
  case ColAcmaEgrisi:
    test.sigortaTipi = value.toString();
    break;
  case ColKutupSayisi:
    test.kutupSayisi = value.toInt();
    break;
  case ColIn:
  ...
  case ColFazKesiti:
    ...
    break;
  case ColNotrKesiti:
    test.notrKesiti = value.toString();
    break;
  case ColToprakKesiti:
    test.toprakKesiti = value.toString();
    break;
  case ColSonuc:
    test.sonuc = value.toString();
    break;
  case ColKakrVar:
    test.kakrVar = value.toBool();
    break;
  case ColRcdAcma:
    test.rcd = value.toString();
    break;
  case ColRcdMa:
    test.rcdMa = value.toString();
    break;
  case ColRcdMs:
    test.rcdMs = value.toString();
    break;
  case ColKakrYok:
    test.kakrYok = value.toBool();
    break;
  default:
    return false;
  }
...
QVariant FonksiyonTestleriModel::headerData(int section,
  ...
  static const QStringList headers = {
      "#",  "Linye", "Eğri",  "Kut", "In",  "Icu", "Ib", "Faz", "N",
      "PE", "Iz",    "Sonuç", "K",   "IΔn", "mA",  "mS", "Yok"};
...
Qt::ItemFlags FonksiyonTestleriModel::flags(const QModelIndex &index) const {
  ...
  if (index.column() != ColIz && index.column() != ColIb &&
...
void FonksiyonTestleriModel::insertAnaSigortaRow() {
  ...
  FonksiyonTesti test;
  test.linye =
      QString::fromUtf8("ANA SİGORTA"); // Python: e.insert(0, "ANA SİGORTA")
  test.sigortaTipi = "C";               // Varsayılan
  test.sonuc = "Uygun";
  test.isAnaSigorta = true; // 32A KAKR kuralından muaf (Python:
                            // entries['_is_ana_sigorta'] = True)
...
void FonksiyonTestleriModel::validateSonuc(int row) {
  ...
  if (!isKakrGroup && test.akimKapasitesi > 0 &&
      test.nominalAkim > test.akimKapasitesi) {
    uygunDegil = true;
    kusur = QString::fromUtf8(
                "Iz(%1A) < In(%2A) - Kablo akım taşıma kapasitesi yetersiz")
                .arg(test.akimKapasitesi)
                .arg(test.nominalAkim);
  }
  ...
  if (!test.isAnaSigorta && test.nominalAkim > 0 && test.nominalAkim <= 32) {
    // 30mA KAKR var mı kontrol et (Python: has_30ma_kakr = kakr_checked and
    // rcd_acma_val == "30mA")
...
void FonksiyonTestleriTable::setupUi() {
  ...
  addBtn->setStyleSheet("background-color: #4CAF50; color: white; font-weight: "
  ...
  groupBtn->setStyleSheet("background-color: #2196F3; color: white; "
                          "font-weight: bold; padding: 6px 12px;");
  connect(groupBtn, &QPushButton::clicked, this,
          &FonksiyonTestleriTable::addLinieGroup);
  toolbar->addWidget(groupBtn);

  QPushButton *insertBtn = new QPushButton(tr("↕ Araya Ekle"));
  insertBtn->setStyleSheet("background-color: #FF9800; color: white; "
                           "font-weight: bold; padding: 6px 12px;");
  connect(insertBtn, &QPushButton::clicked, this,
          &FonksiyonTestleriTable::addMultipleRows);
  toolbar->addWidget(insertBtn);

  QPushButton *testBtn = new QPushButton(tr("● DENE"));
  testBtn->setStyleSheet("background-color: #f44336; color: white; "
                         "font-weight: bold; padding: 6px 12px;");
  connect(testBtn, &QPushButton::clicked, this,
          &FonksiyonTestleriTable::autoFillSelectedRcd);
  toolbar->addWidget(testBtn);

  QPushButton *copyBtn = new QPushButton(tr("📋 Kopyala"));
  copyBtn->setStyleSheet(
      "background-color: #9C27B0; color: white; padding: 6px 12px;");
  connect(copyBtn, &QPushButton::clicked, this,
          &FonksiyonTestleriTable::copySelectedRows);
  toolbar->addWidget(copyBtn);

  QPushButton *pasteBtn = new QPushButton(tr("📥 Yapıştır"));
  pasteBtn->setStyleSheet(
      "background-color: #00BCD4; color: white; padding: 6px 12px;");
  connect(pasteBtn, &QPushButton::clicked, this,
          &FonksiyonTestleriTable::pasteRows);
  toolbar->addWidget(pasteBtn);

  QPushButton *removeBtn = new QPushButton(tr("🗑️ Seçileni Sil"));
  removeBtn->setStyleSheet("background-color: #E91E63; color: white; "
                           "font-weight: bold; padding: 6px 12px;");
  connect(removeBtn, &QPushButton::clicked, this,
          &FonksiyonTestleriTable::removeSelectedRows);
  ...
  m_tableView->setItemDelegateForColumn(
      FonksiyonTestleriModel::ColAcmaEgrisi,
      new ComboBoxDelegate({"B", "C", "D", "K", "Z", "AAA"}, this));
  m_tableView->setItemDelegateForColumn(
      FonksiyonTestleriModel::ColKutupSayisi,
      new ComboBoxDelegate({"1", "2", "3", "4"}, this));
  m_tableView->setItemDelegateForColumn(
      FonksiyonTestleriModel::ColIn,
      new ComboBoxDelegate({"6", "10", "16", "20", "25", "32", "40", "50", "63",
                            "80", "100", "125", "160", "200", "250", "315",
                            "400"},
                           this));
  m_tableView->setItemDelegateForColumn(
      FonksiyonTestleriModel::ColIcu,
      new ComboBoxDelegate(
          {"3kA", "4.5kA", "6kA", "10kA", "25kA", "35kA", "55kA", "70kA"},
          this));
  m_tableView->setItemDelegateForColumn(
      FonksiyonTestleriModel::ColFazKesiti,
      new ComboBoxDelegate({"1.5", "2.5",  "4",    "6",    "10",   "16",  "25",
                            "35",  "50",   "70",   "95",   "120",  "150", "185",
                            "240", "2x16", "2x25", "2x35", "3x70", "3x95"},
                           this));
  m_tableView->setItemDelegateForColumn(
      FonksiyonTestleriModel::ColNotrKesiti,
      new ComboBoxDelegate({"1.5", "2.5", "4", "6", "10", "16", "25", "35",
                            "50", "70", "95", "120", "150"},
                           this));
  m_tableView->setItemDelegateForColumn(
      FonksiyonTestleriModel::ColToprakKesiti,
      new ComboBoxDelegate(
          {"1.5", "2.5", "4", "6", "10", "16", "25", "35", "50"}, this));
  m_tableView->setItemDelegateForColumn(
      FonksiyonTestleriModel::ColSonuc,
  ...
  m_tableView->setItemDelegateForColumn(
      FonksiyonTestleriModel::ColRcdAcma,
  ...
  m_tableView->setSelectionBehavior(
      QAbstractItemView::SelectItems); // Hücre seçimi
  ...
  header->setSectionResizeMode(FonksiyonTestleriModel::ColLinye,
                               QHeaderView::Stretch);
  ...
  m_tableView->setColumnWidth(FonksiyonTestleriModel::ColAcmaEgrisi,
                              45); // Eğri
  m_tableView->setColumnWidth(FonksiyonTestleriModel::ColKutupSayisi,
                              40);                                        // Kut
  ...
  m_tableView->setColumnWidth(FonksiyonTestleriModel::ColToprakKesiti,
                              40);                                   // PE
  ...
  m_tableView->setColumnWidth(FonksiyonTestleriModel::ColKakrVar,
                              30); // K (checkbox)
  ...
  connect(m_model, &FonksiyonTestleriModel::modelDataChanged, this,
          &FonksiyonTestleriTable::dataChanged);
...
void FonksiyonTestleriTable::setupContextMenu() {
  ...
  connect(m_tableView, &QTableView::customContextMenuRequested,
          [this](const QPoint &pos) {
            QMenu menu;
            menu.addAction(tr("Satır Ekle"), this,
                           &FonksiyonTestleriTable::addRow);
            menu.addAction(tr("Seçilenleri Sil"), this,
                           &FonksiyonTestleriTable::removeSelectedRows);
            menu.addSeparator();
            menu.addAction(tr("Linye Grubu Ekle"), this,
                           &FonksiyonTestleriTable::addLinieGroup);
            menu.exec(m_tableView->viewport()->mapToGlobal(pos));
          });
}

void FonksiyonTestleriTable::addRow() { m_model->addRow(); }

void FonksiyonTestleriTable::addMultipleRows() {
  bool ok;
  int count = QInputDialog::getInt(
      this, tr("Satır Ekle"), tr("Eklenecek satır sayısı:"), 5, 1, 100, 1, &ok);
...
void FonksiyonTestleriTable::autoFillSelectedRcd() {
  ...
  for (const QModelIndex &idx : selected) {
    ...
    QString rcdType =
        m_model
            ->data(m_model->index(row, FonksiyonTestleriModel::ColRcdAcma),
                   Qt::EditRole)
            .toString();
    ...
    if (rcdType == "30mA") {
      ...
      double ms =
          QRandomGenerator::global()->bounded(200, 401) / 10.0; // 20.0 - 40.0
      m_model->setData(m_model->index(row, FonksiyonTestleriModel::ColRcdMa),
                       ma);
      m_model->setData(m_model->index(row, FonksiyonTestleriModel::ColRcdMs),
                       QString::number(ms, 'f', 1));
    } else if (rcdType == "300mA") {
      int ma = (QRandomGenerator::global()->bounded(250, 291) / 10) *
...
void FonksiyonTestleriTable::copySelectedRows() {
  ...
  QMessageBox::information(this, tr("Kopyalandı"),
                           tr("%1 satır kopyalandı.").arg(m_clipboard.size()));
}

void FonksiyonTestleriTable::pasteRows() {
  if (m_clipboard.isEmpty()) {
    QMessageBox::warning(this, tr("Uyarı"),
                         tr("Yapıştırılacak veri yok. Önce kopyalayın."));
    return;
  }
...
void FonksiyonTestleriTable::addLinieGroup() {
  ...
  // Signal bağlantısı - "Ekle ve Devam" butonuna basıldığında hemen tabloya
  // ekle
  connect(&dialog, &LinieGroupDialog::testsAdded, this,
          [this](const QVector<FonksiyonTesti> &tests) {
            if (tests.isEmpty())
              return;
  ...
  // Dialog kapansa bile testler zaten eklendi (testsAdded signal'ı ile)
  dialog.exec();
}
...
void FonksiyonTestleriTable::clear() { m_model->clear(); }
