...
LinieGroupDialog::LinieGroupDialog(QWidget *parent) : QDialog(parent) {
...
void LinieGroupDialog::setupUi() {
  ...
  linyeGroup->setStyleSheet(
      "QGroupBox::title { color: #4CAF50; font-weight: bold; }");
  ...
  m_nominalAkim->addItems({"1",   "2",   "3",   "4",   "6",   "10", "13", "16",
                           "20",  "25",  "32",  "40",  "50",  "63", "80", "100",
                           "125", "160", "200", "250", "315", "400"});
  m_nominalAkim->setCurrentText("16");
  connect(m_nominalAkim, &QComboBox::currentTextChanged, this,
          &LinieGroupDialog::onInChanged);
  ...
  QStringList kesitler = {"1.5", "2.5", "4",  "6",   "10",  "16",  "25", "35",
                          "50",  "70",  "95", "120", "150", "185", "240"};

  m_fazKesiti = new QComboBox();
  m_fazKesiti->addItems(kesitler);
  m_fazKesiti->setCurrentText("2.5");
  m_notrKesiti = new QComboBox();
  m_notrKesiti->addItems(kesitler);
  m_notrKesiti->setCurrentText("2.5");
  m_toprakKesiti = new QComboBox();
  m_toprakKesiti->addItems(kesitler);
  m_toprakKesiti->setCurrentText("2.5");

  kesitRow->addWidget(new QLabel(tr("Faz:")));
  kesitRow->addWidget(m_fazKesiti);
  kesitRow->addWidget(new QLabel(tr("N:")));
  kesitRow->addWidget(m_notrKesiti);
  kesitRow->addWidget(new QLabel(tr("PE:")));
  kesitRow->addWidget(m_toprakKesiti);
  ...
  kakrGroup->setStyleSheet(
      "QGroupBox::title { color: #FF9800; font-weight: bold; }");
  ...
  connect(m_kakrIn, &QComboBox::currentTextChanged, this,
          &LinieGroupDialog::onKakrInChanged);
  ...
  m_kakrFazKesiti = new QComboBox();
  m_kakrFazKesiti->addItems(kesitler);
  m_kakrFazKesiti->setCurrentText("6");
  m_kakrNotrKesiti = new QComboBox();
  m_kakrNotrKesiti->addItems(kesitler);
  m_kakrNotrKesiti->setCurrentText("6");
  m_kakrToprakKesiti = new QComboBox();
  m_kakrToprakKesiti->addItems(kesitler);
  m_kakrToprakKesiti->setCurrentText("6");

  kakrKesitRow->addWidget(new QLabel(tr("Faz:")));
  kakrKesitRow->addWidget(m_kakrFazKesiti);
  kakrKesitRow->addWidget(new QLabel(tr("N:")));
  kakrKesitRow->addWidget(m_kakrNotrKesiti);
  kakrKesitRow->addWidget(new QLabel(tr("PE:")));
  kakrKesitRow->addWidget(m_kakrToprakKesiti);
  ...
  rcdGroup->setStyleSheet(
      "QGroupBox::title { color: #2196F3; font-weight: bold; }");
  ...
  addBtn->setStyleSheet("background-color: #2E7D32; color: white; font-weight: "
                        "bold; padding: 10px 20px; border-radius: 4px;");
  ...
  addCloseBtn->setStyleSheet(
      "background-color: #1976D2; color: white; font-weight: bold; padding: "
      "10px 20px; border-radius: 4px;");
...
void LinieGroupDialog::onKakrInChanged(const QString &in) {
  static const QMap<int, QString> inToKesit = {
      {25, "4"},  {32, "6"},  {40, "10"},  {50, "16"},
      {63, "16"}, {80, "25"}, {100, "35"}, {125, "50"}};
  int val = in.toInt();
  if (inToKesit.contains(val)) {
    QString kesit = inToKesit[val];
...
void LinieGroupDialog::addGroup() {
  ...
  // Signal emit - tabloya hemen eklenmesi için
  emit testsAdded(m_pendingTests);
