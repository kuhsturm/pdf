...
PanoTabWidget::PanoTabWidget(int panoIndex, QWidget *parent)
    : QWidget(parent), m_panoIndex(panoIndex) {
  setupUi();
  setupConnections();
}
...
QWidget *PanoTabWidget::setupAnaDagitimPano() {
  ...
  group->setStyleSheet("QGroupBox#anaDagitimGroup { border-top: 1px solid "
                       "#555; margin-top: 5px; padding-top: 5px; }");

  QVBoxLayout *mainLayout = new QVBoxLayout(group);
  mainLayout->setContentsMargins(5, 5, 5, 5);
  mainLayout->setSpacing(3);

  // ===== Satır 0: Proje ve Şema Durumu =====
  QHBoxLayout *row0 = new QHBoxLayout();
  m_chkProjeVar = new QCheckBox(tr("Proje Var mı?"), group);
  row0->addWidget(m_chkProjeVar);

  m_chkTekHatSemasiVar = new QCheckBox(tr("Tek Hat Şeması Var mı?"), group);
  row0->addWidget(m_chkTekHatSemasiVar);
  ...
  m_sebekeTipi = new QComboBox(this);
  m_sebekeTipi->setVisible(false);
  m_enerjiSaglayan = new QLineEdit(this);
  m_enerjiSaglayan->setVisible(false);
  m_trafoGucu = new QLineEdit(this);
  m_trafoGucu->setVisible(false);
  m_sistemGerilimi = new QSpinBox(this);
  m_sistemGerilimi->setVisible(false);
  m_sistemFrekans = new QSpinBox(this);
  m_sistemFrekans->setVisible(false);
  m_topraklamaDirenci = new QLineEdit(this);
  m_topraklamaDirenci->setVisible(false);
  m_sigortaTipiAna = new QComboBox(this);
  m_sigortaTipiAna->setVisible(false);
  m_nominalAkimAna = new QSpinBox(this);
  m_nominalAkimAna->setVisible(false);
  m_rcdBilgisi = new QLineEdit(this);
  m_rcdBilgisi->setVisible(false);
  m_loopPeN = new QLineEdit(this);
  m_loopPeN->setVisible(false);
  m_loopLN = new QLineEdit(this);
  m_loopLN->setVisible(false);
  m_ik3 = new QLineEdit(this);
  m_ik3->setVisible(false);

  // Yeni Eklenen Alanlar (Veri yapısı bütünlüğü için)
  m_rcdAnmaAkimi = new QLineEdit(this);
  m_rcdAnmaAkimi->setVisible(false);
  m_rcdTestBilgisi = new QLineEdit(this);
  m_rcdTestBilgisi->setVisible(false);
  m_distCevrimEmpedansi = new QLineEdit(this);
  m_distCevrimEmpedansi->setVisible(false);
  m_hataAkimi = new QLineEdit(this);
  m_hataAkimi->setVisible(false);
  m_sistemTopraklamaKesiti = new QLineEdit(this);
  m_sistemTopraklamaKesiti->setVisible(false);
  m_anaEspotansiyelKesiti = new QLineEdit(this);
  m_anaEspotansiyelKesiti->setVisible(false);
...
void PanoTabWidget::setupFonksiyonTestleri() {
  ...
  connect(m_fonksiyonTable, &FonksiyonTestleriTable::dataChanged, this,
          &PanoTabWidget::dataChanged);
}
...
QWidget *PanoTabWidget::setupTermalGoruntuler() {
  ...
  m_termalImages->setPlaceholderText(
      tr("Fluke DOCX veya görüntü dosyalarını sürükleyin\nveya çift tıklayarak "
         "seçin"));
  m_termalImages->setMinimumHeight(150);

  connect(m_termalImages, &DragDropWidget::filesChanged, this,
          &PanoTabWidget::dataChanged);
  ...
  connect(clearBtn, &QPushButton::clicked, m_termalImages,
          &DragDropWidget::clear);
...
void PanoTabWidget::setupConnections() {
  ...
  connect(m_panoAdi, &QLineEdit::textChanged, this,
  ...
  connect(m_sebekeTipi, &QComboBox::currentTextChanged, this,
          &PanoTabWidget::dataChanged);
  ...
  connect(m_parafudrTip, &QLineEdit::textChanged, this,
          &PanoTabWidget::dataChanged);
  connect(m_uygunluk, &QComboBox::currentTextChanged, this,
          &PanoTabWidget::dataChanged);

  // Proje ve Şema durumu değiştiğinde
  connect(m_chkProjeVar, &QCheckBox::toggled, this,
          &PanoTabWidget::dataChanged);
  connect(m_chkTekHatSemasiVar, &QCheckBox::toggled, this,
          &PanoTabWidget::dataChanged);

  // OTOMATİK SONUÇ GÜNCELLEMESİ (Python: _check_and_update_sonuc)
  // FT satırı değişince sonucu kontrol et
  connect(m_fonksiyonTable, &FonksiyonTestleriTable::dataChanged, this,
          &PanoTabWidget::checkAndUpdateSonuc);

  // Kritik GK alanı değişince sonucu kontrol et
  connect(m_gozleKontrol, &GozleKontrolWidget::criticalFieldChanged, this,
          &PanoTabWidget::checkAndUpdateSonuc);
}
...
void PanoTabWidget::checkAndUpdateSonuc() {
  ...
  // 2. Kritik GK alanlarını kontrol et (Zemin İzolasyonu, Pano Kapak, Aşırı
  // Yük)
...
PanoData PanoTabWidget::getData() const {
  ...
  data.anaDagitimPano.projeVarMi = m_chkProjeVar->isChecked();
  data.anaDagitimPano.tekHatSemasiVarMi = m_chkTekHatSemasiVar->isChecked();
  ...
  static const QStringList gkOrder = {
      "Kablo Sebeke Tarafi",
      "Pano Sabitlenmesi",
      "Elektrik Panosu Etrafinda Yabanci Malzemeler",
      "Kablo Donanim Tarafi",
      "Dis Darbelere Karsi Koruma Onlemi",
      "Zemin Izolasyonu",
      "Topraklama Iletkeni",
      "Ek Potansiyel Dengeleme Iletkeni",
      "Ana Potansiyel Dengeleme Iletkeni",
      "Pano Kapak Baglantisi Kontrolu 6 mm2",
      "Elektriksel Olmayan Tesislere Yaklasma",
      "Guvenlik Devre Ayrilmasi",
      "Bant Ayrilmasi",
      "Pano Ic Kapak",
      "Semalar Talimatlar",
      "Tehlike Isaretleri",
      "Koruma Cihaz ve Terminal Etiket",
      "Kablo Yollari",
      "Tesisat Yontemi",
      "Kablo Renk Kodlari",
      "Yangin Engeli",
      "Kontak Gevsekligi Isinmasi",
      "Asiri Yuk Isinmasi",
      "",
      "", // GK_24, GK_25 = Fotoğraf tarihi/no (Fluke'dan)
      "Yangin Sondurme",
      "Korozyon Kontrolu",
      "Ekipman Temizlik",
...
void PanoTabWidget::setData(const PanoData &data) {
  ...
  if (idx >= 0)
    m_sebekeTipi->setCurrentIndex(idx);
  ...
  if (idx >= 0)
    m_sigortaTipiAna->setCurrentIndex(idx);
  ...
  // Proje ve Şema Durumu
  m_chkProjeVar->setChecked(data.anaDagitimPano.projeVarMi);
  m_chkTekHatSemasiVar->setChecked(data.anaDagitimPano.tekHatSemasiVarMi);
  ...
  m_re->setText(
      data.anaDagitimPano.distCevrimEmpedansi); // RE = distCevrimEmpedansi
  ...
  if (idx >= 0)
    m_uygunluk->setCurrentIndex(idx);
  ...
  if (idx >= 0)
    m_izoUygunluk->setCurrentIndex(idx);
}
...
void PanoTabWidget::clear() {
  ...
  m_chkProjeVar->setChecked(false);
  m_chkTekHatSemasiVar->setChecked(false);
  // Yeni alanlar
